...
- 정적인 웹이아닌 동적인 웹을 구현하기 위해 request와 response를 입력하고 받아오는 것이 자바코드로 작성하기위해 JSP가 등장했고, 이후 서블릿을 더 간편하고 효율적으로 활용하기 위해 스프링 MVC 프레임워크의 핵심요소인 <code>DispatcherServlet</code>을 사용
  - DispatcherServlet은 서블릿 컨테이너에 포함되어 있음
  - 서블릿 컨테이너는 DispatcherServlet을 생성하고 관리하며, 모든 웹 요청을 DispatcherServlet을 통해 처리하므로 Spring 도 DispatcherServlet 기반으로 동작
...
DispatcherServlet
- 프론트 컨트롤러 패턴
- DispatcherServlet이 모든 요청을 받고, Handler Mapping이 요청에 맞는 컨트롤러를 반환, Handler Adapter가 해당 컨트롤러의 메서드를 호출해 로직을 실행
- 처리 결과를 DispatcherServlet이 받아, View Resolver에 전달하여 해당 응답에 맞는 뷰를 지정
...
- DispatcherServlet은 서블릿 컨테이너에서 실행되지만, 그 본질은 Spring MVC의 핵심 구성 요소로서, Spring에서 HTTP 요청을 처리하는 중앙 진입점 역할을 함
```

<br>

### DispatcherServlet은 Servlet Container 인가 Spring Container 인가

- DispatcherServlet은 서블릿 컨테이너에서 실행되지만, 그 본질은 Spring MVC의 핵심 구성 요소로서, Spring에서 HTTP 요청을 처리하는 중앙 진입점 역할을 함
- 서블릿 컨테이너는 DispatcherServlet을 서블릿으로 등록하고, 클라이언트로부터 들어오는 웹 요청을 이 서블릿에 전달하게됨
- DispatcherServlet은 Spring MVC 프레임워크가 제공하는 서블릿. Spring MVC에서 제공하는 이 서블릿은 모든 웹 요청을 처리하는 중앙 역할을 맡음
  - DispatcherServlet은 Spring MVC 프레임워크가 제공하는 서블릿으로, 서블릿 컨테이너에 의해 관리되면서 Spring 애플리케이션의 요청 처리 로직을 실행함
- 즉, 서블릿 컨테이너가 DispatcherServlet을 실행하지만, DispatcherServlet의 내부 로직은 Spring MVC에서 제공하는 웹 요청 처리 기능을 수행하는 것
