...
- 보통 자주 사용되는 빈이 있기 마련이므로 기본적으로는 @Primary를 더 많이 사용. 모든 빈에 @Qualifier를 붙이기 보다 메인인 @Primary를 기본값으로 주입하고, 예외로 다른 빈을 등록해야하는 경우 @Qualifier를 추가로 사용해서 다른 빈을 주입할 수 있음
- 추가적으로 @Qualifier 이름의 문자열 기반으로 컴파일 타임에 문자열 확인이 안되기 때문에 주로 해당 설정들을 커스텀 인터페이스로 생성하여 사용됨. ( 타입 기반으로 동작하며 컴파일 타임 안정성 확보 가능 )
```
