...
```
WAR : 전통적인 웹 애플리케이션 배포 방식. 기존에는 서버에 톰캣 같은 WAS를 설치하여 빌드 후 배포하다 보니 WAR 파일을 WAS 환경 내에서 실행해야함
JAR : 스프링 부트의 기본 배포 방식. JAR파일 내부에 톰캣 WAS를 내장. 그러다 보니 서버에 따로 WAS 설치할 필요 없고 오직 JAVA만 설치되어 있으면 됨. 파일 용량이 가벼운 것이 아닌 톰캣 설치-환경변수설정-포트설정 같은 인프라 세팅이 필요 없어 배포와 운영이 가볍다는 것
```
