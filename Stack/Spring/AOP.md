...
### AOP 프록시 패턴 ( Target이 인터페이스 상속 )
- AOP는 런타임 시점에 프록시 객체를 생성하여 대상 target 객체 시점 전/후 처리를 진행
    - 프록시객체 : 원본이 아닌(target) 원본과 유사한 핵심 비즈니스 로직을 가진 빈을 감싸고 있는 가짜 객체로 런타임에 target 메서드 호출 전/후 등에 끼어들어서 무언가 부가적인 작업을 하는 객체
    - 클라이언트는 Target을 직접 호출하는 것이 아니라 프록시 객체를 호출하게 되며, 프록시가 중간에 끼어들어(Intercept) 공통 로직(Advice)을 처리한 뒤 실제 Target을 호출함
