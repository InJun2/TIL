...
1. 클라이언트 로그인 요청
2. 서버 요청 사용자 인증
3. 서버에서 성공적으로 인증 시 세션 생성
4. 고유한 세션 ID 'Set-Cookie' 헤더를 통해 클라이언트로 전송
5. 클라이언트는 해당 세션 ID 쿠키로 저장 및 요청마다 해당 세션 ID 전송을 통한 인증 상태 유지
6. 사용자 로그아웃 요청 시 해당 세션 삭제 및 무효화
7. 혹은 세션 타임아웃 시 세션 자동 만료

```scss
1. [클라이언트] --- 로그인 요청 (사용자 이름, 비밀번호) ---> [서버]
2. [서버] --- 사용자 인증 확인 (DB에서 사용자 정보 확인) ---> [DB]
3. [서버] --- 세션 생성, 세션 ID 생성 ---> [서버 메모리]
4. [서버] --- 세션 ID를 클라이언트에게 전달 ---> [클라이언트 쿠키에 세션 ID 저장]
5. [클라이언트] --- 요청 시마다 세션 ID를 서버로 전송 ---> [서버]
6. [서버] --- 세션 ID를 통해 사용자의 세션 확인 ---> [서버]
7. [클라이언트] --- 로그아웃 요청 ---> [서버]
8. [서버] --- 세션 무효화 또는 삭제 ---> [서버 메모리]
```

<br>

### 2. HTTP의 특성인 Stateless
- Stateless(상태 비저장) 프로토콜로 HTTP 요청 간에 서버가 클라이언트의 상태를 유지하지 않는다는 의미
- 각 HTTP 요청은 독립적. 서버는 클라이언트의 이전 요청이나 현재 상태에 대한 정보를 기억하지 않으며 요청마다 새로운 연결로 간주
- 이러한 Stateless는 인증된 사용자인지 기억하지 못하며 상태에 따른 연결하지 못함
- 이러한 문제를 해결하기 위해 세션과 쿠키가 도입되었음
    - 세션은 서버 측에서 클라이언트 상태를 유지하기 위한 방법으로 사용자에 대한 정보를 메모리나 DB가 저장하고 고유한 세션 ID를 통해 사용자의 상태를 추적 및 클라이언트 요청마다 세션 ID를 서버로 전송
    - 쿠키는 클라이언트 측에서 상태를 유지하기 위한 방법으로 서버는 클라이언트에게 쿠키를 생성하여 보낼 수 있으며 클라이언트의 브라우저는 해당 쿠키를 저장하여 요청마다 쿠키를 함께 전송

<br>

### 3. Stateless의 의미를 살펴보면, 세션은 적절하지 않은 인증 방법이지 않은지
- 상태를 유지하기 위해 서버에서 해당 연결을 관리하는 방법으로 Stateless와 충돌하는 부분이 존재
- 그렇기 때문에 세션 기반 인증은 서버가 모든 클라이언트의 상태를 관리하므로 클라이언트가 많을수록 서버 부담 증가
- 세션은 Stateless 측면에서는 적합하지 않을지 몰라도 전통적인 웹 애플리케이션에서는 아직 유효한 방법이며 주로 서버에서 관리해야하는 경우 유용한 방법임
    - 서버 측에서의 세밀한 제어와 보안이 필요한 경우
    - 중앙 집권식 세션관리가 필요한 경우
    - 클라이언트 측 저장소의 제약이 있을 경우
    - 자주 갱신되는 사용자 데이터가 있는 경우

<br>

### 4. 규모가 커져 서버가 여러 개가 됐을 때 세션 관리 방법
- 분산서버가 되었을 경우 모든 서버가 사용자의 세션 상태를 공유하며 관리하여야 함
- 세션 복제 사용
    - 각 서버에서 생성된 세션이 다른 모든 서버로 복제하는 방법
    - 서버 장애가 발생해도 다른 서버에서 세션을 계속 사용할 수 있음
    - 서버가 많아지면 세션 복제에 따른 네트워크 트래픽 증가 및 세션 데이터가 클 경우 성능에 부정적인 영향
- 중앙 세션 저장소 사용
    - 모든 서버가 공통의 중앙 저장소 (Redis, Memcached, RDBMS 등)에 세션 데이터를 저장하고 조회하는 방식
    - 서버들은 이 중앙 저장소를 통해 세션 데이터를 공유
    - 세션 데이터를 중앙에서 관리하여 각 서버의 리소스 부담이 줄어들고 서버 간의 세션 데이터 일관성을 유지할 수 있음
    - 하지만 모든 요청이 중앙 세션 저장소로 도달하여 병목이 되고 장애가 발생시 모든 서버에서 세션 관리가 중단됨
- 토큰 기반 인증
    - 세션 상태를 서버에서 관리하지 않고 JWT 를 클라이언트 측에서 상태 정보를 저장하는 방식
    - 서버가 상태를 저장할 필요가 없어 서버 간 동기화 문제가 발생하지 않음
    - JWT가 클라이언트에 저장되므로 토큰 탈취문제 발생 가능
    - 결과적으로 세션이 아닌 클라이언트 측 저장으로 토큰을 조기에 무효화하거나 갱신하기 어려움
