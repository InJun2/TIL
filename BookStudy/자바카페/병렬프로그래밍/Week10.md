...
## 16. 자바 메모리 모델
### 흥미로운 부분
- 자바 언어 명세에서는 JVM이 단일 스레드 내부에서는 순차적으로 동작하는 것과 동일하게 실행되며 이 경우 같은 결과를 만들어주기만 한다면 여러 가지 기법을 사용해도 문제가 없음
    - 병렬 처리 프로그램에서도 스레드 간의 작업을 조율하는데 꼭 필요한 데이터만을 공유해 사용하는 것이 올바른 방법
- 책에서 나오는 파이프라인 슈퍼스칼라 기법과 섬세한 다중 메모리 캐시란?
    - 파이프라인 슈퍼스칼라 기법은 파이프라인과 슈퍼스칼라 기법을 합친 방법으로 파이프라이닝을 통해 명령어의 각 단계를 병렬로 처리하고 슈퍼스칼라 아키텍처로 병렬 처리 내부에서 여러 명령어를 동시에 실행할 수 있음
    - 섬세한 다중 메모리 캐시는 다중 메모리 캐시는 CPU 성능을 향상시키기 위해 여러 단계의 캐시를 사용하는 기법
- 시스템 구조에서 말하는 메모리 모델은 프로그램이 메모리 구조에서 어느 정도의 기능을 사용할 수 있을지에 대한 정보를 제공하고 메모리의 내용을 공유할때 프로세서 간의 작업을 조율하기 위한 특별한 명령어가 무엇이 있고 어떻게 사용하는지 정보를 제공함
- 동기화 피기백이란?
    - 분산 시스템에서 서로 다른 노드 간에 데이터 일관성을 유지하기 위해 사용되는데 노드간의 관계 정보를 제공하여 각 노드간 데이터 일관성을 유지
    - FutureTask, BlockingQueue 등 에서 안전한 공개 기법이 피기백의 한 형태라고 함 ( 큐에 값을 집어넣는 스레드와 값을 꺼내가는 스레드 간의 내부 적절한 동기화 )
- 동기화 피기백에서 안전한 공개를 위한 미리 발생 규칙은 개별적인 메모리 작업의 수준에서 일어나는 순서의 문제를 정의하여 동기화를 진행하므로 데이터 일관성과 스레드 안전성을 유지할 수 있음
    - 어셈블리어로 다루는 방법은 복잡하고 오류를 발생하기 쉬움
- 'static synchronized'가 추가적인 스레드 안전성을 제공하는 이유는 클래스 자체에 락이 걸려 클래스의 모든 인스턴스가 동일한 락을 공유하기 때문
    - 'synchronized'만 있다면 클래스에 락이 걸리기보다는 인스턴스 수준에서 락이 걸린다고 함
    - static 구문은 JVM에서 해당 클래스를 읽어들이고 실제 해당 클래스를 사용하기 전에 실행되어 JVM이 락을 확보한 상태에서 메모리에 쓰여진 내용은 모든 스레드가 볼 수 있음
...
- 책에서 자바에서 동기화를 문서화하기 위한 어노테이션은 JSR(Java Specification Request)-305의 어노테이션으로 그 외 동기화 문서화(모두 개발자가 직접 정의)를 위한 다른 어노테이션은 다음이 존재
