...
### OpenSSL
- SSL은 복잡한 바이너리 프로토콜이므로 암호 전문가가 아닌이상 가공되지 않은 SSL 트래픽을 직접 보내면 안됨
...
- 다이제스트 인증은 비밀번호 자체를 보호하는 것 일뿐이고 그 외의 어떤 비밀 보호를 제공하는 것은 아니기 때문에 TLS와 HTTPS보다 안전한 프로토콜이라 볼 수는 없음
