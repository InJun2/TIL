...
- 동기적 인터럽트는 현재 수행 중인 프로세스나 스레드가 직접적으로 발생시킨 이벤트로 발생한 인터럽트. 프로세스나 스레드가 이벤트를 발생시켜 대응이 필요한 인터럽트
- 비동기적 인터럽트는 외부에서 발생한 이벤트에 의해 발생하여 현재 작업과 무관하게 발생할 수 있음. 외부에서 발생하여 언제 발생할지 예측할 수 없고 해당 인터럽트를 처리하는 방식에 대해 대비책을 갖춰야 함
